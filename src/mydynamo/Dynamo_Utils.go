...
//Initial VectorClock with specific node
func NewVectorClockWithNode(nodeID string) VectorClock{
	clock := NewVectorClock()
	clock.Increment(nodeID)
	return clock
}

func NewPutArgsWithClock(key string, nodeID string, value[] byte) PutArgs{
	newClock:= NewVectorClock()
	if len(nodeID) != 0{
		newClock = NewVectorClockWithNode(nodeID)
	}
	newContext := NewContext(newClock)
	return NewPutArgs(key, newContext, value)
}
