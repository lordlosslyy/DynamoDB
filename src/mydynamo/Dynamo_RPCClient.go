...
func (dynamoClient *RPCClient) PutInternal(value PutArgs) bool {
	var result bool
	if dynamoClient.rpcConn == nil {
		return false
	}
	err := dynamoClient.rpcConn.Call("MyDynamo.PutInternal", value, &result)
	if err != nil {
		log.Println(err)
		return false
	}
...
func (dynamoClient *RPCClient) GetInternal(key string) *DynamoResult {
	var result DynamoResult
	if dynamoClient.rpcConn == nil {
		return nil
	}
	err := dynamoClient.rpcConn.Call("MyDynamo.GetInternal", key, &result)
	if err != nil {
		log.Println(err)
		return nil
	}
